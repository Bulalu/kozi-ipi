...
def _():
    ...
    from kozi_analysis.reporting import (
        render_candidate_gate_markdown,
        write_candidate_comparison_reports,
        write_candidate_gate_reports,
    )

    class CandidateGateParams(BaseModel):
        ...
        write_report: bool = Field(
            default=False,
            description="Whether to write candidate-gate.md/JSON.",
        )
        fail_on_blockers: bool = Field(
            default=False,
            description="Exit with an error when the gate has blockers.",
        )
    ...
    def params_from_cli(raw_args: dict[str, Any]) -> CandidateGateParams:
        ...
        return CandidateGateParams(**normalized)

    def print_usage() -> None:
        print("Usage: uv run notebooks/08_candidate_gate.py [options]")
        print()
        for name, field in CandidateGateParams.model_fields.items():
...
@app.cell
def _(CandidateGateParams, mo, params_from_cli, print_usage):
...
def _(
    ...
    return candidate_dir, comparison, expected_changes_path, gate, report_dir
...
def _(
    ...
    gate_markdown = render_candidate_gate_markdown(gate)
    return gate_markdown, output_dir


@app.cell
def _(candidate_dir, expected_changes_path, gate, mo, output_dir):
    mo.md(
        f"""
        ## Summary
        ...
        - Expected differences: **{gate.expected_count}**
...
@app.cell
def _(gate_markdown, mo):
    mo.accordion({"Rendered report": mo.md(gate_markdown)})
    return
