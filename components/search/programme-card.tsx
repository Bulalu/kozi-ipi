...
export function ProgrammeCard({
  ...
  const admissionRouteLabel = formatAdmissionRoutes(programme.entryRouteTypes)
          ...
          <div className="min-w-0 flex-1">
            <div className="flex min-w-0 flex-wrap items-center gap-1.5 text-[10.5px] font-semibold tracking-[0.11em] text-brand-ink/45 uppercase sm:gap-2 sm:text-[11.5px] sm:tracking-[0.14em]">
              <span className="max-w-full min-w-0 break-words">
                {formatProgrammeAward(programme.awardLevel)}
              </span>
      ...
      {programme.minimumEntryRequirements ? (
        <div className="mt-4 rounded-xl bg-[#fafafa] p-3.5">
          <div className="flex min-w-0 flex-wrap items-center gap-2">
            ...
            {admissionRouteLabel ? (
              <span className="max-w-full truncate rounded-full bg-brand-blue/10 px-2 py-0.5 text-[10.5px] font-semibold text-brand-blue">
                Route: {admissionRouteLabel}
              </span>
            ) : null}
          </div>
...
function ProgrammeDetails({ programme }: { programme: ProgrammeSearchResult }) {
  ...
  const detailRows = [
    ...
    {
      label: "Programme award",
      value: formatProgrammeAward(programme.awardLevel),
    },
    ...
    {
      label: "Admission routes",
      value: formatAdmissionRoutes(programme.entryRouteTypes),
    },
...
function formatProgrammeAward(awardLevel: string) {
  const normalized = awardLevel.trim().toLowerCase()
  if (normalized === "degree") return "Degree programme"
  if (normalized.includes("diploma")) return "Diploma programme"
  if (normalized.includes("certificate")) return "Certificate programme"
  return awardLevel
}

function formatAdmissionRoutes(entryRouteTypes?: string) {
  const routes = entryRouteTypes
    ?.split(/[;,|]/)
